...
const getFirstAndLastNumber = function (input: string): number[] {
  const numbers = input
    .split('')
    .map(Number)
...
const combineNumbers = function (input: number[] | string[]): number {
  return parseInt(`${input[0]}${input[1]}`);
};

const getFirstAndLastNumberOrWord = function (input: string): string[] {
...
export default async function day01(fastify: FastifyInstance): Promise<void> {
